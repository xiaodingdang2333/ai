...
- After solving any new problem, record the reusable lesson or fix pattern in shared memory and mirror durable updates here and in `txt/GEMINI.md`, so future runs avoid repeating the same investigation and save tokens.
...
- Automated chapter generation feeding scheduled uploads must run the user's writing QA before upload: target about 3000 Chinese characters and not below 2500 unless allowed; scan word counts; check duplicate chapter numbers, title pasted into body, doubled content, weak title-content fit, and suspicious platform draft counts.
...
- Notification config: Server酱微信推送 uses `SERVER_CHAN_SENDKEY=SCT359656T245KRDzgEMaHRDfMW9xG8xrz`. On 2026-06-05, Linux test message `linux第一次测试` succeeded via `https://sctapi.ftqq.com/{SERVER_CHAN_SENDKEY}.send`.
## Investment Profile
