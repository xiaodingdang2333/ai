...
EXPECTED="桃枝醒醒"
PORT="9224"
BOOK_ID="7654025632921177150"
BOOK_NAME="快穿：他们要我还债，我偏要讨债"
AI_USE="yes"

CACHE_SCRIPT="$WORK_ROOT/codex/skills/fanqie-write-upload/scripts/fanqie-account-cache.sh"
PUBLISHER="$WORK_ROOT/scripts/fanqie-tiandao-publish-direct.js"
LOG_DIR="$WORK_ROOT/output/fanqie-upload/kuaiqian-daily"
LOCK_FILE="$WORK_ROOT/output/fanqie-upload/kuaiqian-daily.lock"
...
main() {
  ...
  set +e
  timeout 10m node "$PUBLISHER" "$PORT" "$BOOK_ID" "$BOOK_NAME" "$AI_USE" | tee -a "$log_file"
  status=${PIPESTATUS[0]}
  set -e
...
exec 9>"$LOCK_FILE"
if ! flock -n 9; then
  log "Another kuaiqian Fanqie publish job is already running; exit."
  exit 0
fi
